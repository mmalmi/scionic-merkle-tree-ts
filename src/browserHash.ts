/**
 * Browser-compatible hashing utilities
 * Uses Web Crypto API instead of Node's crypto module
 */

import * as cbor from 'cbor';

/**
 * Hash data using SHA256 (Web Crypto API)
 */
...
/**
 * Create a CID from CBOR-encoded data (browser-compatible)
...
export async function createCID(data: any): Promise<string> {
  ...
  // Hash with SHA256 (browser-compatible)
  const hash = await hashData(new Uint8Array(encoded));
